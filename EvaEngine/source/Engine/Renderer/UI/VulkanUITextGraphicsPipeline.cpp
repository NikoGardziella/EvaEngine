...
namespace Engine {
    ...
    VulkanUITextGraphicsPipeline::VulkanUITextGraphicsPipeline(VkDevice device, VkPhysicalDevice physicalDevice)
    ...
    VulkanUITextGraphicsPipeline::~VulkanUITextGraphicsPipeline()
    {
        Shutdown();
    }
    ...
    void VulkanUITextGraphicsPipeline::Init(const UIInitConfig& cfg, uint32_t framesInFlight)
    {
        ...
        m_uiTextShader = std::make_shared<VulkanShader>(AssetManager::GetAssetPath("shaders/VulkanUITextShader.GLSL").string());

        CreatePipelineLayout();
        CreatePipeline(cfg);
    }

    void VulkanUITextGraphicsPipeline::Shutdown()
    {
        ...
        if (m_uiTextShader->GetVertexShaderModule() != VK_NULL_HANDLE)
        {
            vkDestroyShaderModule(m_device, m_uiTextShader->GetVertexShaderModule(), nullptr);

        }

        if (m_uiTextShader->GetFragmentShaderModule() != VK_NULL_HANDLE)
        {
            vkDestroyShaderModule(m_device, m_uiTextShader->GetFragmentShaderModule(), nullptr);
        }
    ...
    VkDescriptorSet VulkanUITextGraphicsPipeline::GetCameraDescriptorSet(uint32_t frame) const
    {
        EE_CORE_ASSERT(frame < m_setCamera.size(), "GetCameraDescriptorSet: frame out of range");
        return m_setCamera[frame];
    }

    VkDescriptorSet VulkanUITextGraphicsPipeline::GetUITextureDescriptorSet(uint32_t frame) const
    {
        EE_CORE_ASSERT(frame < m_setUITextures.size(), "GetUITextureDescriptorSet: frame out of range");
        return m_setUITextures[frame];
    }

    void VulkanUITextGraphicsPipeline::UpdateCameraUBO(uint32_t frame, const CameraUBO& data)
    {
    ...
    void VulkanUITextGraphicsPipeline::UpdateUITextures(uint32_t frame, const std::vector<Ref<VulkanTexture>>& textures)
    {
    ...
    void VulkanUITextGraphicsPipeline::CreateCameraBuffers(uint32_t framesInFlight)
    {
    ...
    void VulkanUITextGraphicsPipeline::DestroyCameraBuffers()
    {
    ...
    void VulkanUITextGraphicsPipeline::CreatePipelineLayout()
    {
        ...
        pcr.offset = 0;
        pcr.size = sizeof(Engine::VulkanUIGraphicsPipeline::UIPushConstants); 
    ...
    void VulkanUITextGraphicsPipeline::FillVertexInputState(VkPipelineVertexInputStateCreateInfo& vi,
        ...
        std::array<VkVertexInputAttributeDescription, 4>& attrs) const
    {
        ...
        bindings[0].stride = sizeof(Engine::VulkanUIGraphicsPipeline::VulkanUIQuadVertex);
        bindings[0].inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

        attrs[0].location = 0;
        attrs[0].binding = 0;
        attrs[0].format = VK_FORMAT_R32G32B32_SFLOAT;
        attrs[0].offset = offsetof(Engine::VulkanUIGraphicsPipeline::VulkanUIQuadVertex, Position);


        attrs[1].location = 1;
        attrs[1].binding = 0;
        attrs[1].format = VK_FORMAT_R32G32B32A32_SFLOAT;
        attrs[1].offset = offsetof(Engine::VulkanUIGraphicsPipeline::VulkanUIQuadVertex, Color);

        attrs[2].location = 2;
        attrs[2].binding = 0;
        attrs[2].format = VK_FORMAT_R32G32_SFLOAT;
        attrs[2].offset = offsetof(Engine::VulkanUIGraphicsPipeline::VulkanUIQuadVertex, TexCoord);


        attrs[3].location = 3;
        attrs[3].binding = 0;
        attrs[3].format = VK_FORMAT_R32_UINT;
        attrs[3].offset = offsetof(Engine::VulkanUIGraphicsPipeline::VulkanUIQuadVertex, TexIndex);
    ...
    void VulkanUITextGraphicsPipeline::CreatePipeline(const UIInitConfig& cfg)
    {
        ...
        stages[0].module = m_uiTextShader->GetVertexShaderModule();
        stages[0].pName = "main";

        stages[1].sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
        stages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT;
        stages[1].module = m_uiTextShader->GetFragmentShaderModule();
        stages[1].pName = "main";
    ...
    uint32_t VulkanUITextGraphicsPipeline::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const
    {
